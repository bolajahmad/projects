...
summary: >
  A smart contract development tooling for interacting with runtime primitives deployed on,
...
<figcaption>Architecture representation</figcaption>
</figure>
...
The primary engineering challenge is maintaining protocol consistency across all development environments. WHile the CLI and SDK exist in this repository, the actual precompile implementation lives inside the runtime.

These environments represent data differently, and most importantly, include certain crates that have to be version-bound. The versions of these crates change with the runtime code, but the SDK has to consistently monitor the runtime to be ready to upgrade in case of changes.

The project therefore focuses less on implementing cryptographic algorithms themselves, which already exists in the runtime, and more on guaranteeing that developers can incoke those capabilities safely through deterministic SDK abstractions.
...
The system is primarily optimized, not only for runtime execution, but also developer productivity.

By replacing handwritten payload with strongly typed helper libraries, the toolkit reduces integration complexity. eliminates repeated encoding across projects, and shortens the feedback loop during contract development.
