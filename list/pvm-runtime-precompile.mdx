...
PVM Precompiles is a developer toolkit for exposing high-value
cryptographic precompiles to smart contracts running on PVM-enabled
parachains. The project pairs a Solidity developer surface with a
Rust-based CLI and crate layer so teams can generate payloads,
validate encodings, and integrate precompile calls without manually
handling runtime byte layouts.

The long-term goal is to make major and useful precompiles accessible
through a familiar Solidity library and a companion CLI,
while helping bring Polkadot Revive style runtime capabilities
